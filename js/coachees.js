...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (!coacheesContainer || !modal || !form) {
        console.error("Éléments HTML critiques pour la page 'Mes Coachés' non trouvés. Le script ne peut s'exécuter.");
        return;
    }
        ...
        return `
            <div class="coachee-card bg-white rounded-lg shadow-lg overflow-hidden flex flex-col transition-transform hover:transform-gpu hover:-translate-y-1" data-id="${coachee.id}">
                ...
                <div class="p-3 bg-gray-50 border-t border-gray-200 flex justify-end gap-2">
                    <button data-action="edit" class="text-gray-500 hover:text-blue-600 p-2 rounded-full" title="Modifier"><i class="fas fa-edit"></i></button>
                    <button data-action="delete" class="text-gray-500 hover:text-red-600 p-2 rounded-full" title="Supprimer"><i class="fas fa-trash"></i></button>
    ...
    function handleFormSubmit(event) {
        ...
        const coacheeId = coacheeIdInput.value;
        if (coacheeId) { // Mode Édition
            coachees = coachees.map(c => c.id === coacheeId ? { ...c, ...coacheeData } : c);
        } else { // Mode Ajout
            coachees.push({
                ...coacheeData,
                id: 'coachee-' + Date.now(),
